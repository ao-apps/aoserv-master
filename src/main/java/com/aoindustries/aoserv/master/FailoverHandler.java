 ...
 * Copyright (C) 2001-2013, 2015, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2026  AO Industries, Inc.
...
public final class FailoverHandler implements CronJob {
  ...
  public static void setFileReplicationBitRate(
    ...
    if (bitRate == null) {
      conn.update("update backup.\"FileReplication\" set max_bit_rate=null where id=?", fileReplication);
    } else {
      conn.update("update backup.\"FileReplication\" set max_bit_rate=? where id=?", bitRate, fileReplication);
    }
  ...
  public static void setFileReplicationSchedules(
    ...
    for (int c = 0; c < size; c++) {
      ...
      if (existingPkey == -1) {
        ...
        if (!ids.removeByValue(existingPkey)) {
          throw new SQLException("ids doesn't contain id=" + existingPkey);
        }
      }
    }
  ...
  public static Server.DaemonAccess requestReplicationDaemonAccess(
    ...
    if (quotaGid == -1) {
      ...
      if (!isQuotaEnabled) {
        throw new SQLException("quota_gid is not null when quota_enabled=false: backup.FileReplication.id=" + fileReplication);
      }
    }

    HostAddress connectAddress = conn.queryObjectNullable(
