 ...
 * Copyright (C) 2001-2013, 2015, 2017, 2018, 2019, 2020, 2021, 2022, 2024, 2025, 2026  AO Industries, Inc.
...
public final class PaymentHandler /*implements CronJob*/ {
  ...
  public static int addCreditCard(
    ...
    if (encryptionFrom != -1) {
      checkAccessEncryptionKey(conn, source, "addCreditCard", encryptionFrom);
    }
    if (encryptionRecipient != -1) {
      checkAccessEncryptionKey(conn, source, "addCreditCard", encryptionRecipient);
    }
  ...
  public static void updateCreditCardNumberAndExpiration(
    ...
    if (encryptionFrom != -1) {
      checkAccessEncryptionKey(conn, source, "updateCreditCardNumberAndExpiration", encryptionFrom);
    }
    if (encryptionRecipient != -1) {
      checkAccessEncryptionKey(conn, source, "updateCreditCardNumberAndExpiration", encryptionRecipient);
    }
  ...
  public static void setCreditCardUseMonthly(
    ...
    if (creditCard == -1) {
      ...
      if (!account.equals(getAccountForCreditCard(conn, creditCard))) {
  ...
  public static void paymentSaleCompleted(
    ...
    if (updated != 1) {
      throw new SQLException("Unable to find payment.Payment with id=" + payment + " and status in ('PROCESSING', 'AUTHORIZED')");
    }
  ...
  public static void paymentAuthorizeCompleted(
    ...
    if (updated != 1) {
      throw new SQLException("Unable to find payment.Payment with id=" + payment + " and status='PROCESSING'");
    }
  ...
  private static void synchronizeStoredCards(PrintWriter verboseOut, PrintWriter infoOut, PrintWriter warningOut, boolean dryRun) throws IOException, SQLException {
    try (
      ...
      try (DatabaseConnection conn = MasterDatabase.getDatabase().connect()) {
        InvalidateList invalidateList = new InvalidateList();
        if (infoOut != null) {
          infoOut.println(PaymentHandler.class.getSimpleName() + ".synchronizeStoredCards: Synchronizing stored cards");
        }
