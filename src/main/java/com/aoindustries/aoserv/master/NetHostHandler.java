...
public final class NetHostHandler {
  ...
  public static int addBackupServer(
    ...
    if (!AccountHandler.isAccountOrParent(conn, account, farm_owner)) {
      throw new SQLException("Not able to access farm: "+farm);
    }
    ...
    if (PasswordChecker.hasResults(Locale.getDefault(), results)) {
      throw new SQLException("Password strength check failed: "+PasswordChecker.getResultsString(results).replace('\n', '|'));
    }
    ...
    while (true) {
      ...
      if (currentAccount.equals(AccountHandler.getRootAccount())) {
        break;
      }
