...
public final class AccountUserHandler {
	...
	public static void addUser(
		...
		if(name.equals(com.aoindustries.aoserv.client.linux.User.MAIL)) throw new SQLException("Not allowed to add User for name '"+com.aoindustries.aoserv.client.linux.User.MAIL+'\'');

		if(!avoidSecurityChecks) {
			PackageHandler.checkAccessPackage(conn, source, "addUser", packageName);
			if(PackageHandler.isPackageDisabled(conn, packageName)) throw new SQLException("Unable to add User '" + name + "', Package disabled: " + packageName);
			...
			if(atPos!=-1) {
				...
				if(hostname.length()>0) MasterServer.checkAccessHostname(conn, source, "addUser", hostname);
			}
		}
	...
	public static void disableUser(
		...
		if(isUserDisabled(conn, user)) throw new SQLException("Username is already disabled: "+user);
	...
	public static void removeUser(
		...
		if(user.equals(source.getCurrentAdministrator())) throw new SQLException("Not allowed to remove self: "+user);
	...
	public static void removeUser(
		...
		if(user.equals(com.aoindustries.aoserv.client.linux.User.MAIL)) throw new SQLException("Not allowed to remove User named '"+com.aoindustries.aoserv.client.linux.User.MAIL+'\'');
