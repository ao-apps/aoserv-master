...
public final class TicketHandler /*implements Runnable*/ {
  ...
  public static boolean canAccessTicketAction(DatabaseConnection conn, RequestSource source, int action) throws IOException, SQLException {
    ...
    if (mu != null) {
      ...
      if (isTicketAdmin(conn, source)) {
        ...
        if (account != null && AccountHandler.canAccessAccount(conn, source, account)) {
          return true;
        }
  ...
  public static int generateTicketId(
    ...
    for (int range = 1000000; range < 1000000000; range *= 10) {
      for (int attempt = 0; attempt < 1000; attempt++) {
        ...
        if (conn.queryBoolean("select (select id from ticket.\"Ticket\" where id=?) is null", ticket)) {
          return ticket;
        }
      }
    }
  ...
  public static int addTicket(
    ...
    if (account == null) {
      // if (!isAdmin) {
      //   throw new SQLException("Only ticket administrators may create a ticket without a business.");
      // }
    } else {
      AccountHandler.checkAccessAccount(conn, source, "addTicket", account);
      if (AccountHandler.isAccountDisabled(conn, account)) {
        throw new SQLException("Unable to add Ticket, Account disabled: " + account);
      }
    }
  ...
  public static String getTicketInternalNotes(
    ...
    if (isTicketAdmin(conn, source)) {
      return conn.queryString("select internal_notes from ticket.\"Ticket\" where id=?", ticket);
    } else {
      return "";
    }
  }
  ...
  public static void bounceTicket(
    ...
    if (!isActive(conn, ticket)) {
      throw new SQLException("Ticket no longer active: "+ticket);
    }
    UsernameHandler.checkAccessUsername(conn, source, "bounceTicket", username);
    if (username.equals(User.MAIL)) {
      throw new SQLException("Not allowed to bounce a Ticket as user '"+User.MAIL+'\'');
    }
  ...
  public static void changeTicketAdminPriority(
    ...
    if (username.equals(User.MAIL)) {
      throw new SQLException("Not allowed to change a Ticket's admin priority as user '"+User.MAIL+'\'');
    }
  ...
  public static void setTicketAssignedTo(
    ...
    if (username.equals(User.MAIL)) {
      throw new SQLException("Not allowed to assign a Ticket as user '"+User.MAIL+'\'');
    }
...
*/
  public static boolean setTicketAccount(
    ...
    if (newAccount != null) {
      AccountHandler.checkAccessAccount(conn, source, "setTicketBusiness", newAccount);
    }

    int updateCount;
    if (oldAccount == null) {
      updateCount = conn.update("update ticket.\"Ticket\" set accounting=? where id=? and accounting is null", newAccount, ticket);
    } else {
      updateCount = conn.update("update ticket.\"Ticket\" set accounting=? where id=? and accounting=?", newAccount, ticket, oldAccount);
    }
  ...
  */
  ...
  public static void reactivateTicket(
    ...
    UsernameHandler.checkAccessUsername(conn, source, "holdTicket", username);
    if (username.equals(User.MAIL)) {
      throw new SQLException("Not allowed to reactivate Ticket as user '"+User.MAIL+'\'');
    }
  ...
  public static void ticketWork(
    ...
    if (username.equals(User.MAIL)) {
      throw new SQLException("Not allowed to submit Ticket work as user '"+User.MAIL+'\'');
    }
  ...
  public static String getContactEmails(DatabaseConnection conn, int ticket) throws IOException, SQLException {
    ...
    if (contactEmails.length() == 0) {
      return AccountHandler.getTechnicalEmail(conn, getAccountForTicket(conn, ticket));
    }
