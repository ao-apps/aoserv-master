	...
	public static void getSlaveStatus(
		...
		if(conn.executeBooleanQuery("select ao_server is not null from failover_mysql_replications where pkey=?", failoverMySQLReplication)) {
			...
			UnixPath toPath = conn.executeObjectQuery(
				...
				"select bp.path from failover_mysql_replications fmr inner join failover_file_replications ffr on fmr.replication=ffr.pkey inner join backup_partitions bp on ffr.backup_partition=bp.pkey where fmr.pkey=?",
