				...
				if(masterUser != null) {
					...
					if(masterServers.length == 0) {
						...
						MasterServer.writeObjects(
							...
							"select * from \"net/reputation\".\"ReputationLimiterSet\""
						);
					} else if(masterUser.isRouter()) {
						// Router may access all limiters in the same server farm
						MasterServer.writeObjects(
							...
							+ "  inner join net.\"Device\" nd on se2.pkey=nd.server\n"                                   // Find all net.Device in the same farm
							+ "  inner join \"net/reputation\".\"ReputationLimiter\" irl on nd.pkey=irl.net_device\n"    // Find all limiters in the same farm
							+ "  inner join \"net/reputation\".\"ReputationLimiterSet\" irls on irl.pkey=irls.limiter\n" // Find all limiters sets in the same farm
							+ "where\n"
							+ "  ms.username=?",
							username
						);
				...
				if(masterUser != null) {
					...
					if(masterServers.length == 0) {
						...
						MasterServer.writeObjects(
							...
							"select * from \"net/reputation\".\"ReputationLimiter\""
						);
					} else if(masterUser.isRouter()) {
						// Router may access all limiters in the same server farm
						MasterServer.writeObjects(
				...
				if(masterUser != null) {
					...
					if(masterServers.length == 0) {
						...
						MasterServer.writeObjects(
							...
							"select * from \"net/reputation\".\"ReputationSetHost\""
						);
					} else if(masterUser.isRouter()) {
						...
						MasterServer.writeObjects(
							...
							+ "  inner join \"net/reputation\".\"ReputationSetHost\"    irsh on irs.pkey      = irsh.\"set\"\n"   // Find all hosts belonging to these sets
							+ "where\n"
							+ "  ms.username=?",
							username
						);
					...
					MasterServer.writeObjects(
						...
						+ "  \"net/reputation\".\"ReputationSetHost\" irsh\n"
						+ "where\n"
						+ "  un.username=?\n"
						+ "  and un.package=pk.name\n"
						+ "  and (\n"
				...
				if(masterUser != null) {
					...
					if(masterServers.length == 0) {
						...
						MasterServer.writeObjects(
							...
							provideProgress,
							new IpReputationSet(),
							"select * from \"net/reputation\".\"ReputationSet\""
						);
					} else if(masterUser.isRouter()) {
						// Router may access all sets used by any limiters in the same server farm
						MasterServer.writeObjects(
					...
					MasterServer.writeObjects(
						...
						+ "      billing.\"Package\" pk,\n"
						+ BU1_PARENTS_JOIN
						+ "      \"net/reputation\".\"ReputationSet\" irs2\n"
						+ "    where\n"
						+ "      un.username=?\n"
						+ "      and un.package=pk.name\n"
						+ "      and (\n"
						...
						+ "      irs3.pkey\n"
						+ "    from\n"
						+ "      \"net/reputation\".\"ReputationSet\" irs3\n"
						+ "    where\n"
			...
			case MYSQL_SERVER_USERS :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						provideProgress,
						new MySQLServerUser(),
						"select * from mysql.\"MysqlUserServer\""
					); else MasterServer.writeObjects(
						conn,
						source,
						out,
