		...
		switch(tableID) {
			...
			case AO_SERVERS :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						+ "from\n"
						+ "  master_servers ms\n"
						+ "  inner join server.\"AoServer\" ao on ms.server=ao.server\n"
						// Allow its failover parent
						+ "  left join server.\"AoServer\" ff on ao.failover_server=ff.server\n"
						// Allow its failover children
						+ "  left join server.\"AoServer\" fs on ao.server=fs.failover_server\n"
			...
			case DISABLE_LOG :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						+ "  master_servers ms,\n"
						+ "  server.\"AoServer\" ao\n"
						+ "  left join server.\"AoServer\" ff on ao.server=ff.failover_server,\n"
						+ "  business_servers bs,\n"
			...
			case IP_ADDRESSES :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						+ "    from\n"
						+ "      master_servers ms\n"
						+ "      left join server.\"AoServer\" ff on ms.server=ff.failover_server,\n"
						+ "      net_devices nd\n"
						...
						+ "              inner join server.\"AoServer\" bpao on bp.ao_server=bpao.server\n" // Only allow access to the device device ID for failovers
						+ "            where\n"
						+ "              ms.server=ffr.server\n"
			...
			case MYSQL_SERVER_USERS :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						provideProgress,
						new MySQLServerUser(),
						"select * from mysql.\"MysqlServerUser\""
					); else MasterServer.writeObjects(
						conn,
						source,
						out,
			...
			case NET_DEVICES :
				if(masterUser != null) {
					...
					//+ "  left join backup.\"FileReplication\" ffr on bs.server=ffr.server\n"
					//+ "  left join backup.\"BackupPartition\" bp on ffr.backup_partition=bp.pkey\n"
					//+ "  left join server.\"AoServer\" bpao on bp.ao_server=bpao.server,\n"
					+ "  net_devices nd\n"
					+ "where\n"
					+ "  un.username=?\n"
					+ "  and un.package=pk.name\n"
			...
			case SERVERS :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						+ "from\n"
						+ "  master_servers ms\n"
						+ "  left join server.\"AoServer\" ao on ms.server=ao.server\n"
						// Allow its failover parent
						+ "  left join server.\"AoServer\" ff on ao.failover_server=ff.server\n"
						// Allow its failover children
						+ "  left join server.\"AoServer\" fs on ao.server=fs.failover_server\n"
