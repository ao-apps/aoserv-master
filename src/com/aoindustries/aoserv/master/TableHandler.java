			...
			case HTTPD_SITE_BIND_REDIRECTS :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						"select * from httpd_site_bind_redirects"
					); else MasterServer.writeObjects(
						conn,
						source,
						out,
			...
			case IP_ADDRESSES :
				if(masterUser != null) {
					...
					if(masterServers.length == 0) MasterServer.writeObjects(
						...
						+ "  ia.\"isAlias\",\n"
						+ "  ia.hostname,\n"
						+ "  (select pk.name from packages pk where pk.pkey = ia.package),\n"
						+ "  ia.created,\n"
						+ "  ia.available,\n"
						+ "  ia.\"isOverflow\",\n"
						+ "  ia.\"isDhcp\",\n"
						+ "  m_ia.\"pingMonitorEnabled\",\n"
						+ "  ia.\"externalInetAddress\",\n"
						+ "  ia.netmask,\n"
						+ "  m_ia.\"checkBlacklistsOverSmtp\",\n"
						+ "  m_ia.enabled\n"
						+ "from\n"
						+ "  \"IPAddress\" ia\n"
						+ "  inner join monitoring.\"IPAddress\" m_ia on ia.id=m_ia.id"
					); else MasterServer.writeObjects(
						conn,
						source,
						out,
